...
class Task:
    ...
    task:str        # 任务内容
    status:TaskStartus      # 状态：未开始， 进行中， 已完成， 推迟
    priority:int    # 优先级
...
class UserTaskList:
    ...
    def load_task(self, L:list[dict]):
        for d in L:
            task = Task(d['create_time'], d['deadline'], d['task'],d['status'], d['priority'])
    ...
    def save(self):
        ...
        for task in self.task_list:
            print(task.task)
            data_list.append({'create_time':task.create_time, 'deadline':task.deadline, 
                              'task':task.task, 'status':task.status, 'priority':task.priority})
        with open(LOCALDATA, 'r', encoding='utf-8') as rf:
            json_obj = json.load(rf)
            data = json_obj[self.name]
            data = data_list
